            ...
            .then(progress ->
                proxy.initiate("AWS-CloudFormation-Stack::Delete", proxyClient, progress.getResourceModel(), progress.getCallbackContext())
                    ...
                    .makeServiceCall((awsRequest, client) -> {
                        DeleteStackResponse awsResponse;
                        try {
                            logger.log(String.format("%s %s", this.getClass(), Optional.ofNullable(request.getClientRequestToken()).orElse("")));
                            awsResponse = client.injectCredentialsAndInvokeV2(awsRequest, client.client()::deleteStack);
                    ...
                    .stabilize((awsRequest, awsResponse, client, model, context) -> {
                        ...
                        try {
                            describeStacksResponse = client.injectCredentialsAndInvokeV2(DescribeStacksRequest.builder()
                                .stackName(model.getStackId()).build(), client.client()::describeStacks);
                        } catch (final AwsServiceException e) {
